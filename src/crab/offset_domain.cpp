...
bool offset_domain_t::check_packet_access(const Reg& r, int width, int offset) const {
    ...
    if (it) {
        ...
        bound_t init_limit = crab::bound_t(crab::number_t(PACKET_BEGIN));
        bound_t final_limit = crab::bound_t(crab::number_t(PACKET_BEGIN));

        if (meta_limit) init_limit = meta_limit.value()-crab::number_t(PACKET_META);
        ...
        if (dist_from_begin.m_dist.lb()+crab::number_t(offset) >= init_limit
                && dist_from_begin.m_dist.lb()+crab::number_t(offset)+bound_t(width) <= final_limit)
            return true;
    }
    return false;
}
