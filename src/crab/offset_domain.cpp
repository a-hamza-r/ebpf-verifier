...
void dist_t::write(std::ostream& o) const {
    ...
    if (is_meta_pointer())
        ...
        o << "end-" << interval_t(crab::number_t(-1))*m_dist-interval_t(crab::number_t(4099));
    }
}
...
bool dist_t::is_meta_pointer() const {
    return (m_dist.lb() >= crab::number_t(-4098)
            && m_dist.ub() < crab::number_t(0));
}
bool dist_t::is_forward_pointer() const {
    return (m_dist.lb() >= crab::number_t(0));
}
bool dist_t::is_backward_pointer() const {
    return (m_dist.ub() < crab::number_t(4098));
}
...
packet_constraint_t packet_constraint_t::operator|(const packet_constraint_t& other) const {
    ...
    dist_t b = dist_t(weight_t(crab::number_t(-4099)));
...
ctx_t::ctx_t(const ebpf_context_descriptor_t* desc) {
    if (desc->data != -1) {
        m_dists[desc->data] = dist_t(weight_t(crab::number_t(0)));
    }
    if (desc->end != -1) {
        m_dists[desc->end] = dist_t(weight_t(crab::number_t(-4099)));
    }
    if (desc->meta != -1) {
        m_dists[desc->meta] = dist_t(weight_t(crab::number_t(-1)));
    }
    m_size = desc->size;
}
    ...
    if (std::holds_alternative<Reg>(bin.v)) {
        ...
        switch (bin.op)
        {
            ...
            case Bin::Op::ADD: {
                ...
                if (src_const_value) {
                    ...
                    if (dst_dist.m_dist.lb() >= crab::number_t(0)) {
                        updated_dist = dst_dist.m_dist + src_const_value.value();
                    }
                    else if (dst_dist.m_dist.lb() >= crab::number_t(-4098)) {
        ...
        switch (bin.op)
        {
            case Bin::Op::ADD: {
                ...
                weight_t updated_dist;
                interval_t imm_interval{crab::number_t(imm)};
                if (dst_dist.m_dist.lb() >= crab::number_t(0)) {
                    updated_dist = dst_dist.m_dist + imm_interval;
                }
                else if (dst_dist.m_dist.lb() >= crab::number_t(-4098)) {
                    // TODO: special handling of meta pointer required
                    updated_dist = dst_dist.m_dist - imm_interval;
...
bool offset_domain_t::check_packet_access(const Reg& r, int width) const {
    ...
    if (it && meta_limit && end_limit) {
        ...
        if (dist.m_dist.lb() >= meta_limit.value()
                && dist.m_dist.lb()+crab::number_t(width) <= end_limit.value()) return true;
    }
    return false;
}
