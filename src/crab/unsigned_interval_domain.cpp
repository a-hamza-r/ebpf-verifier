...
void unsigned_interval_registers_t::insert(register_t reg, const location_t& loc, interval_t interval) {
...
std::optional<mock_interval_t> unsigned_interval_registers_t::find(register_location_t reg) const {
    auto it = m_registers_env->find(reg);
    if (it == m_registers_env->end()) return {};
    return it->second;
}

std::optional<mock_interval_t> unsigned_interval_registers_t::find(register_t key) const {
...
void unsigned_interval_stack_t::store(uint64_t key, mock_interval_t val, int width) {
...
void unsigned_interval_stack_t::remove_overlap(const std::vector<uint64_t>& keys, uint64_t start, int width) {
    for (auto& key : keys) {
        auto type = find(key);
        auto width_key = type.value().second;
        if (key < start) {
...
static inline void join_stack(const unsigned_interval_stack_t& stack1, uint64_t key1, int& loc1,
        ...
        unsigned_interval_stack_cells_t& interval_values_joined) {
...
unsigned_interval_stack_t unsigned_interval_stack_t::operator|(const unsigned_interval_stack_t& other) const {
    ...
    return unsigned_interval_stack_t(std::move(interval_values_joined));
}
...
unsigned_interval_domain_t unsigned_interval_domain_t::operator|(const unsigned_interval_domain_t& other) const {
    ...
    return unsigned_interval_domain_t(m_registers | other.m_registers,
