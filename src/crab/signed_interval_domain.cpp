...
bool registers_signed_state_t::operator<=(const registers_signed_state_t& other) const {
    for (uint8_t i = 0; i < NUM_REGISTERS-1; i++) {
        ...
        auto it2 = other.find(*(m_cur_def[i]));
        if (it1 && it2) {
...
bool stack_slots_signed_state_t::operator<=(const stack_slots_signed_state_t& other) const {
