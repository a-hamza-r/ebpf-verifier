...
class expression_t {
  ...
  public:
    expression_t() : _interval(interval_t::top()) {};
    expression_t(symbol_terms_t symbol_terms, interval_t interval = interval_t{0})
        : _symbol_terms(symbol_terms), _interval(interval) {}
    expression_t(symbol_t symbol) : _interval(interval_t{0}) {
        _symbol_terms[symbol] = 1;
    }
    expression_t(interval_t interval)
        : _symbol_terms(symbol_terms_t{}), _interval(interval) {}
    expression_t(int n)
        : _symbol_terms(symbol_terms_t{}), _interval(interval_t{n}) {}
    ...
    interval_t get_interval() const { return _interval; }
